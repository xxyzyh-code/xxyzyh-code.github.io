...
<div id="site-stats" style="text-align:center; margin-bottom:30px; font-size:1.1em; color:#888; background:#f5f5f5; padding:15px 10px; border-radius:10px;">
  ...
  {% for post in site.posts %}
    ...
    {% assign last_update = post.last_modified_at | default: post.date | date: "%Y-%m-%d" %}
  {% endfor %}

  <p>📊 全站写作统计：</p>
  <p>
    总文章数：<strong>{{ post_count }}</strong> | 
    总字数：<strong>{{ total_words }}</strong> | 
    平均每篇字数：<strong>{% if post_count > 0 %}{{ total_words | divided_by: post_count | round }}{% else %}0{% endif %}</strong><br>
    一级分类数：<strong>{{ cat_set | size }}</strong> | 
    二级分类数：<strong>{{ subcat_set | size }}</strong> | 
    最后更新：<strong>{{ last_update }}</strong>
  </p>
</div>
